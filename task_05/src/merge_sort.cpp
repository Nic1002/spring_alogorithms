...
std::vector<int> _slice(const std::vector<int>& vector, const size_t from, const size_t to)
{
    if (from < to) 
...
{   
    ...
    if (i_l >= size_l) {
        for (size_t i = i_r; i < size_r; ++i) 
            nw.push_back(right[i]);
        return nw;
    }
    if (i_r >= size_r) {
        for (size_t i = i_l; i < size_l; ++i) 
            nw.push_back(left[i]);
        return nw;
    }
    return nw;
