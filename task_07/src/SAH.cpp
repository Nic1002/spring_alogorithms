...
std::pair<double, double> KDTree::SAH(Point left_min_point,
  ...
  double width = get_axis(right_max_point, curr_axis) -
                 get_axis(left_min_point, curr_axis);
  ...
  for (int index = range.first; index <= range.second; ++index) {
    int bin_index = static_cast<int>((get_axis(cloud[index], curr_axis) -
                                      get_axis(left_min_point, curr_axis)) /
                                     bin_width);
  ...
  for (int i = 1; i < bins; ++i) {
    double x0 = get_axis(left_min_point, curr_axis) + width * i / bins;
    double SAl = (x0 - get_axis(left_min_point, curr_axis)) * width_w;
    double SAr = (get_axis(right_max_point, curr_axis) - x0) * width_w;
